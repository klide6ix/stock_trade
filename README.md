...
- [ ] 🔴 **장전 갭 신호가 0% 로 들어가는 결함 수정** — 2026-08-03 08:25~08:28 실관측에서 확인. 개장 전인데도 KIS 일봉에 **오늘 봉이 전일 종가 그대로 채워진 채 이미 존재**해(`stck_bsop_date`=오늘, 종가=전일종가) `judge_direction` 의 `has_today_bar` 가 True 가 되고, `_gap_signal` 이 장전 예상체결가 경로 대신 **장중 실시간 등락률 경로**를 탄다. 개장 전 실시간 등락률은 +0.00% 이므로 **가중치 0.50 짜리 갭 신호가 '0' 이라는 값으로 점수에 들어가** 판정을 0 쪽으로 희석시킨다(실측: 점수 -0.247, 갭을 빼고 재정규화하면 -0.493 — 확신도가 정확히 절반). 갭이 '미사용' 되는 것보다 나쁜 상태다. 수정안: `has_today_bar` 판정에 **오늘 봉의 거래량 > 0**(또는 `now >= 09:00`)을 추가해, 실제 거래가 시작된 뒤에만 실시간 등락률 경로를 타게 한다. 부수 확인: 기준가(108,820)는 전일 종가와 **일치**했고 `장운영구분코드`=112 였다 — stale 판정 자체는 통과하므로 예상체결가 경로만 살리면 갭 신호를 쓸 수 있다(08:30 이후 예상거래량 형성 여부는 재관측 필요)
- [ ] **장전 예상체결가 갭 신호 실장 검증 (재관측 필요)** — 평일 08:30~09:00 에 실호출해 `기준가(stck_sdpr)` 가 당일 세션 기준으로 롤오버되는 시점 확인. 2026-08-03 08:25~09:02 에 [scripts/_watch_premarket.py](scripts/_watch_premarket.py) 로 1분 간격 관측(기준가 롤오버·예상거래량 형성 시점·갭 사용 여부·예상체결가 vs 실제 시가 오차)을 시작했고, 기록은 `data/premarket_watch_YYYY-MM-DD.jsonl`. **개장 직후 최종 재판정 도입으로 최악의 경우(끝까지 stale)에도 갭이 실시간 등락률로 살아나 리스크는 낮아졌지만**, 장전 30분 동안 방향을 알 수 있는지는 이 관측이 답한다. 끝까지 stale 이면 판정 기준을 `장운영구분코드(antc_mkop_cls_code)` 기반으로 교체 검토
