...
- [ ] **중립 밴드(`neutral_band`) 표본 외 검증** — 0.18~0.25 에서 거래를 절반으로 줄이며 전체 +90.72%(Sharpe +4.44 · MDD -8.65%)를 냈으나, ridge 가 좁고(0.15·0.30 에서 급락) 필터 후 표본이 30여 거래뿐이다. 고변동에서만 듣고 저변동에서는 오히려 나빠지는 점도 확인 필요. 10월 이후 데이터로 train/test 분할 재검증 후 채택 여부 결정. 현재 기본값 0(항상 진입) 유지
...
- [ ] 오버나이트 꼬리 리스크를 **포지션 사이즈**로 관리 검토 — 청산 시점을 앞당기는 것(당일 청산)은 σ 배수 청산선에서도 자산곡선 Sharpe 를 떨어뜨리는 것으로 재확인됐으므로(+1.96 → +1.56), 줄이려면 배정 자금(`short_term_budget`)을 낮추는 쪽이 맞다. 다만 동일 변동성 환산 시 두 모드가 거의 동률(+0.37% vs +0.34%)이라 **사이즈 축소의 이점도 이전 추정만큼 크지는 않다** — 현재 300만원이 계좌에서 차지하는 비중 기준으로 적정선 산정 필요
