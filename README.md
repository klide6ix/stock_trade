...
| 매수 후보 목록   | 시가총액 상위 100 → 거래량 상위 20 → 주간 상승률 상위 5종목 (트레이더 시작 시 1회 탐색) |
| 거래 이력        | 매수/매도 시각, 체결가, 수량, 메모 (최신순) |
| 거래량 상위 5    | 실시간 거래량 순위 표시                                                           |
...
실행
 ...
 │     └─ VolumeMomentumBuyStrategy: 시총 상위 100 → 거래량 상위 20 → EPS 음수 제외 → PER*EPS·주간등락률 순위 합산 상위 5
 │
 └─ 10분마다 반복 (장 운영시간 내)
      └─ 보유 종목 전체 조회
