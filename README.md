...
- [ ] **단기 매매 청산선(-5%) 재조정 — 최우선.** 7월 시뮬레이션([scripts/_simulate_july.py](scripts/_simulate_july.py))에서 **이 설정이 실손익을 지배**함이 확인됐다. 300만원 시드·22거래일 기준, 현행 `-5%/-5%` 는 청산 20회 중 **17회가 최고가 청산**이라 결과가 장중 경로 가정에 따라 **-22.26% ~ +10.48%** 로 갈려 판정조차 불가. 청산선을 넓히면 장중 청산이 사라져 두 경로가 **수렴**한다 — `최고가 -10%/손절 -10%` +26.72%~+50.34%, `최고가 -15%/손절 -10%` 는 보수·낙관 모두 **+47.25%**(경로 독립). **권장: 사이드바에서 `short_term_peak_drop_pct` 5 → 10, `short_term_stop_loss_pct` 5 → 10 — 코드 변경 불필요.** -15% 는 7월 중 최고가 청산이 한 번도 발동하지 않아 사실상 비활성과 같으므로, 보호장치를 남기려면 -10% 가 적정. 추후 실현변동성 배수(예: `2.5 × vol`) 기반 동적 청산선 검토
