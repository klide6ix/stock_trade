...
- [ ] **개장판정 로그 관찰 (수정 효과 확인)** — 위 재판정 재시도 수정이 실제로 듣는지는 며칠치 `[개장판정]` 로그로만 확정된다. 확인할 것: ① 09:00 첫 시도에서 '실측 갭 미반영' 이 실제로 찍히는가(= 08-12 가설의 직접 증거) ② 몇 번째 주기에 `GAP_SOURCE_LIVE` 로 넘어가는가 ③ 09:05 까지 끝내 실패하는 날이 있는가. ③ 이 잦으면 `OPEN_REJUDGE_UNTIL` 을 늘리거나 일봉 대신 `get_quote_snapshot` 으로 장중 여부를 판정하는 쪽을 검토한다. 로그 보관이 5일(`logger.RETENTION_DAYS`)이라 **주 단위로 들여다봐야** 놓치지 않는다
