...
- [x] **장 운영 시간 외 '대기 중' 로그 정리** — `while` 루프가 매 사이클(60초) 한 줄씩 찍어 **평일 1,020줄 · 주말 1,440줄**이 쌓였고, 보관 5일 기준 약 **5,940줄**이 정작 봐야 할 매매·판정 기록을 덮었다(사후 추적이 로그 5일치뿐이라 그만큼 손해). 대기 구간 **진입 시 1회 + `IDLE_LOG_INTERVAL`(60분) 마다 한 줄**로 줄였다 — 같은 기준 약 **104줄**.
  - 프로세스 생존 신호는 유지한다(완전 제거가 아니라 heartbeat 화). 줄에 **다음 장전 준비 시각**(`pre_market_open_time()`)을 붙여 대기 상태에서도 정보를 준다
  - 장 시간·장전 분기로 넘어가면 타이머를 `None` 으로 리셋해, 다음 대기 구간(장 마감 후)에 들어갈 때 다시 한 줄이 남는다
  - 회귀 3건 추가 ([scripts/_test_rejudge.py](scripts/_test_rejudge.py) 5절 — 149분 대기에 정확히 3줄 · 장전 준비 시각 표기 · 장 마감 후 재진입 시 재기록). `run_loop` 에 기동 시각(`start`)·로그 수집(`log_sink`) 인자를 추가해 검증 가능하게 했다

- [x] **확신 비례 사이징 재검증 (표본 +8거래일) — 결론이 오히려 선명해졌다, 채택 근거 없음** (2026-09-24, 사용자 제안으로 재실행. `_check_trade_frequency.END` 를 09-11 → 09-23 으로 확장, 표본 79거래일).
  - **동기와 사실 확인** — "판정이 애매한 날은 배정액의 50%만 사면 어떤가". 계기가 된 09-23 은 확인해 보니 **확신 상위 7위 / 80거래일**(백분위 91% · 점수 **+0.488** · 갭 +1.95%, |점수| 중앙값은 0.169)이었다. **확신이 가장 높은 축의 날이라 이 방식으로는 막히지 않는 손실**이다 — 그날은 시가 114,290 이 곧 그날 고점(고가 114,305)이고 저가 111,945 까지 밀린 **갭 되돌림** 이었다(확신도가 아니라 진입 타이밍 문제, 아래 '개장 직후 평균회귀' 후보와 같은 계열)
  - **원리가 약해졌다** — |점수| 5분위 적중률 **67% / 44% / 44% / 69% / 75%** 로 **단조성이 사라졌다**(Q1 이 오히려 높다). corr(|점수|, 수익률) **+0.181 · t = +1.61 로 유의하지 않다**(직전 +0.222 · t +2.00). Q1 이 튀는 것은 앞선 '경계선 날은 기댓값 0 의 무작위' 관찰과 일관된다 — 무작위라 표본에 따라 위아래로 흔들린다
  - **국면 의존은 그대로** — 고변동 5~7월 **+3.23%p** vs 저변동 8~9월 **-0.06%p**(직전 +0.04%p). 지금 국면에서 효과는 0 이거나 음수다
  - **🔴 대조군에 더 크게 진다** (test 36거래일, 직전 28일):

    | test(8~9월) 규칙 | 수익률 | Sharpe | MDD |
    | ---------------- | ------ | ------ | --- |
    | 고정 100% (현행) | +21.93% | +3.73 | -9.79% |
    | 고정 75% (대조군) | +16.41% | **+3.79** | -7.35% |
    | 확신 2단계 사이징 | +13.60% | **+2.99** | -6.32% |

  - **해석** — 고정 비율 축소는 Sharpe 를 바꾸지 못하므로(스케일 불변), 확신 사이징이 **같은 평균 투입비율(73%)의 고정 규칙보다 Sharpe 가 높아야** 정보를 실제로 쓴 것이다. 수익·Sharpe 모두 지므로 **'확신 낮은 날 절반' 은 결국 돈을 덜 넣은 것일 뿐**이다. train(5~7월)에서는 여전히 압도적이지만(+46.94% · Sharpe +4.36 vs 대조군 +25.41% · +2.71), 그 우위가 표본 외에서 재현되지 않는다
  - **손실 폭을 줄이고 싶다면 수단은 따로 있다** — 배정 자금 하향(아래 '🎯 리스크 줄이기' 항목). 고정 비율 축소는 Sharpe 가 그대로라 **효율 손해 없이 MDD 만 비례 축소**된다. 이건 전략 변경이 아니라 리스크 선호 설정이라 표본 외 검증도 필요 없다
...
- [ ] **확신 기반 사이징 재검증 (10월 이후)** — 2026-09-24 재검증(test 36거래일)에서 **결론이 더 선명해졌다**: 5분위 단조성 소멸(67/44/44/69/75%) · corr +0.181(t +1.61, 유의하지 않음) · 저변동 국면 효과 **-0.06%p** · test 에서 고정 75% 대조군에 수익·Sharpe 모두 패배(+13.60%/+2.99 vs +16.41%/+3.79). 그래도 기각이 아니라 **판정 보류**인 이유는 train(고변동)에서의 우위가 뚜렷하고(+46.94% · Sharpe +4.36) test 가 아직 36거래일뿐이기 때문이다 — 다음 고변동 국면이 와야 원리가 국면 의존인지 소멸인지 갈린다. 10월 이후 데이터가 쌓이면 `scripts/_check_conviction.py` 재실행 — 저변동 국면에서도 상위 분위가 이기면 채택, 아니면 '고변동 전용' 으로 조건부 적용 검토. 채택 시 on/off 밴드보다 **2단계 사이징**(|점수|≤0.20 절반)이 임계값 민감도가 낮아 우선
