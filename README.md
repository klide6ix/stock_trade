...
- [x] 검증 — 순수 로직 38건(대체 ETF 회피 · 4중 청산 경계값 · 재진입 차단 · 원장 분리 · 일단위 갱신) + 모의 API 트레이더 사이클 27건(개장 진입 → 최고가 추적 → 최고가 청산 → 당일 차단 → 익일 보유기간 청산 → 방향 전환 재진입 · 겹침 시 대체 ETF · 외부 청산 감지 · 체결가 보정 · **자금 풀 손익 누적**: 손실 -36,000원 → 풀 2,964,000원 / 이익 +15,000원 → 풀 3,015,000원 → 다음 진입 603주(캡 방식이었다면 600주) · **체결 정산**: 주문가 10,000원 → 실제 체결 10,020원·제비용 450원이 원장·풀에 정확히 반영) 전부 통과. 실 KIS 조회로 방향 판정 동작 확인(📉 하락 -0.887 → 인버스 후보 3종), 예상체결가 stale 판정이 정상 작동함을 실측 확인

## 다음 작업 후보

- [ ] **장전 예상체결가 갭 신호 실장 검증** — 평일 08:30~09:00 에 실호출해 `기준가(stck_sdpr)` 가 당일 세션 기준으로 롤오버되는 시점 확인. 롤오버가 09:00 이후라면 stale 판정에 항상 걸려 갭 신호가 영구 미사용되므로, 그 경우 판정 기준을 `장운영구분코드(antc_mkop_cls_code)` 기반으로 교체
- [ ] 방향 판정 가중치·정규화 기준(`W_*`, `NORM_*_PCT`) 튜닝 — 실운용 로그를 모아 상승/하락 적중률 대비 조정. 현재 값은 코스피200 일간 변동폭 기준의 초기 추정치
- [ ] 중립 밴드(`neutral_band`) 사이드바 노출 — 방향 확신이 낮은 날(점수 ±0.1 이내) 진입을 건너뛰는 옵션. 현재 0(항상 진입)
