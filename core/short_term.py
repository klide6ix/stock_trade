"""단기간 매매 (단타) 전략 — 코스피200 근사 풀에서 등락률+거래량 ranking 결합 1종목 추적.
...
선정 흐름 (KIS API 3회 호출, 일봉 조회 없음 — 즉시 완료):
  ...
  4. 두 ranking 모두에 등장하는 종목 → ranking 합산이 작을수록(=양쪽에서 상위) 상위. 최상위 1종목.

매도 기준: 매수 평균가 대비 stop_loss_pct (default 5%) 이상 하락.
초기화 단위: 일단위. selected_at 의 날짜가 오늘과 다르면 트레이더가 재선정 트리거.
...
class ShortTermStrategy:
    ...
    def find_target(
        ...
        """등락률 ranking + 거래량 ranking 결합으로 1종목 선정.
        ...
        in_kospi_label = "KOSPI200 시총상위" if best["in_kospi"] else "KOSPI200"
        log(
            f"[단타] ranking 결합 후보 {len(candidates)}개 → 최종 선정: "
            f"{best['종목명']}({best['종목코드']}) — 등락률 {best['등락률순위']}위 / "
            f"거래량 {best['거래량순위']}위 / +{best['등락률(%)']:.2f}% · {in_kospi_label}"
        )
        return {
            "종목코드": best["종목코드"],
            "종목명": best["종목명"],
            "현재가": best["현재가"],
            "선정사유": (
                f"등락률 {best['등락률순위']}위 · 거래량 {best['거래량순위']}위 · "
                f"+{best['등락률(%)']:.2f}% · {in_kospi_label}"
            ),
        }
...
def target_to_settings(
    ...
    """`find_target()` 결과를 settings 저장 포맷으로 변환.
...
def needs_reselection(slot: dict[str, Any] | None, today: datetime | None = None) -> bool:
    """슬롯이 비어 있거나, 다른 날짜에 선정된 슬롯이면 재선정 필요.

    일단위 초기화 — selected_at 의 날짜와 오늘이 다르면 새로 선정한다.
    """
    if not is_target_set(slot):
        return True
    selected_at = slot.get("selected_at") if isinstance(slot, dict) else None
    if not selected_at:
        return True
    try:
