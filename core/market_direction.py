...
def _gap_signal(
    ...
    개장 전이면 동시호가 예상체결가를 쓴다. **양쪽 다 stale 검사를 거친다** — 예상체결가는
    직전 세션 잔존값이 올 수 있어 기준가 대조 + 예상거래량 > 0 을, 장중 스냅샷은 개장 직후
    미갱신분을 걸러내려 등락률 ≠ 0 + 시가 > 0 을 요구한다.
    """
    if has_today_bar:
        try:
        ...
        open_px = float(snap.get("시가", 0) or 0)
        # **개장 직후 스냅샷 미갱신 판별** — 일봉이 이미 오늘 것으로 바뀌어
        # `_today_bar_is_live()` 를 통과했는데도, `inquire-price` 의 현재가는 아직 전일
        # 종가 그대로 오는 몇 초가 있다. 그러면 등락률이 **+0.00%** 로 오고, 가중치 0.50
        # 짜리 갭이 '0' 이라는 값으로 점수에 들어가 나머지 신호를 정확히 절반으로 희석시킨다.
        #
        # 2026-09-21 실측: 09:00:03 판정에서 실제 개장 갭이 +0.79%(109,285 → 110,185)
        # 였는데 스냅샷 등락률은 0.00% 였다. 갭을 0 으로 넣은 점수 -0.142 가 운영 판정과
        # 완전히 일치했고(하락 → 인버스 진입), 실측 갭을 넣으면 +0.007(상승)이었다.
        #
        # 갭이 진짜로 0.00% 인 날을 함께 걸러내지만 손해가 아니다 — 갭 0 은 방향 정보가
        # 없다는 뜻이고, **미사용으로 떨어뜨려 나머지 가중치로 재정규화하는 편이 더 정확**
        # 하다(`_today_bar_is_live` 주석의 논리 그대로: 0 으로 들어가면 그 0 이 전체
        # 가중치의 절반을 차지하며 신호를 지운다). 개장 직후 재판정은 이 경우 마킹을
        # 미루고 `OPEN_REJUDGE_UNTIL` 까지 다시 시도한다(`trader.run`).
        #
        # 시가(`stck_oprc`)도 함께 본다 — 체결이 시작되면 확정되는 값이라, 0 이면
        # 스냅샷이 오늘 세션을 아직 반영하지 않았다는 독립적인 증거다.
        if chg == 0.0 or open_px <= 0:
            return None, "장중 스냅샷 미갱신", (
                f"{proxy.name} 등락률 {chg:+.2f}% · 시가 {open_px:,.0f}"
            )
